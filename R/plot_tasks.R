make_site_reach_plots <- function(outind, reaches_to_check, network_latlon, category, ...) {
  ...
  task_makefile_name <- sprintf('network_plots_%s_tasks.yml', category)
  plot_step <- create_task_step(
    step_name = 'plot',
    target_name = function(task_name, ...) {
      sprintf("out/plots/%s_%s_plot.png", category, task_name)
    },
  ...
  create_task_makefile(
    ...
    final_targets = sprintf('out/plots/%s_plots.zip.ind', category),
    as_promises = TRUE
  )
...
subset_and_plot_reach <- function(outfile, reach_and_sites_ind, network_latlon,
                                  seg_id_to_plot,
                                  category) {
  reach_and_sites <- readRDS(as_data_file(reach_and_sites_ind))
