...
site_reach_distance_plot <- function(outind, sites_reaches_ind, bin_width = NULL) {
  reaches_matched <- readRDS(as_data_file(sites_reaches_ind))
  ggplot(data = reaches_matched, aes(x = offset)) +
    geom_histogram(binwidth = bin_width) +
    labs(title = "Distribution of site offsets from initially matched reach*",
         subtitle = sprintf("Bin width = %s", bin_width),
